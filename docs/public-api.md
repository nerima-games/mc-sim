...
**この却下は正しい**。公開済み mc-kernel の `ITEM_TYPES` にも入っていないため、local code で
追加していない。
...
これは公開済み `ClockPort` を直接利用する現在の契約とは**別物**である。
従来の問題は**形の不一致**（1 フィールドのミラーの Layer が 2 フィールドの Tag を満たし、
...
**mc-kernel の公開語彙には入れていない。** kernel の `identifiers.ts` にエンティティ ID は無く、
台帳が mc-sim のものである以上、その鍵も mc-sim のものである —— kernel が kernel 側の理由で
公開する日までは（`ItemType` のときと同じ順序）。
