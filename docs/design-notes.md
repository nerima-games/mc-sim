...
削った（[public-api.md](./public-api.md) §4.1-7）。前者を選ぶと、tier-2 のレシピ表という
**挙動でも計測でもないもの**を根拠に tier-1 の語彙が決まることになり、DN-11 が守ろうとしている
「名前の出所は 1 か所」という性質を、名指し分岐ではなくロスタの側から崩す。
