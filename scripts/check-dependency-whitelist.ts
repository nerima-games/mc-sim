 ...
 *
 * ---------------------------------------------------------------------------
 * For mc-sim the authoritative row is
 *   mc-physics, mc-save, mc-worldgen   (+ mc-kernel, universally)
 *
 * Note what is NOT there. mc-worldgen depends on mc-noise, and mc-sim depends
 * on mc-worldgen — but mc-sim may not import mc-noise. Terrain values reach the
 * simulation as `Chunk` data through mc-worldgen's API, never by mc-sim
 * re-deriving them from a noise function; if it did, two implementations of
 * "what is at this coordinate" would exist and would drift.
 *
 * mc-render is likewise absent, and its absence is the load-bearing one:
 * mc-render depends on mc-sim, so the reverse edge would be an outright cycle.
 * This is the structural expression of plan.md §3.8's camera-ownership
 * inversion fix — mc-sim owns `CameraPoseSnapshot` and cannot ask the renderer
 * anything, because it cannot even see the renderer.
 * ---------------------------------------------------------------------------
 */
export const REPOSITORY_POLICY = {
  thisPackage: '@nerima-games/mc-sim',
